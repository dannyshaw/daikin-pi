...
class Pulses_struct(ctypes.Structure):
    _fields_ = [("gpioOn", ctypes.c_uint32), ("gpioOff", ctypes.c_uint32),
...
class Waveform():
    ...
    def one(self, duration):
        ...
        for i in range(total_pulses):
            if i % 2 == 0:
                self.add_pulse(1 << self.protocol.master.gpio_pin, 0,
                               on_duration)
            else:
                self.add_pulse(0, 1 << self.protocol.master.gpio_pin,
                               off_duration)
...
class IR():
    ...
    def send_frames(self, frames):
        ...
        if chain != 0:
            print 'Error Creating Wave Chain'
            return 1
...
#     protocol = "RC-5"
#     gpio_pin = 17
#     protocol_config = dict(one_duration=820, zero_duration=820)
